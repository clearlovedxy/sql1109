...
show functions get_json_object;

desc function get_json_object;

select get_json_object('[{"name":"大郎","sex":"男","age":"25"},{"name":"西门庆","sex":"男","age":"47"}]', '$[0].name');
select get_json_object(
               '{"common":{"ar":"110000","ba":"iPhone","ch":"Appstore","md":"iPhone 8","mid":"mid_515","os":"iOS 13.3.1","uid":"391","vc":"v2.1.132"},"displays":[{"displayType":"query","item":"4","item_type":"sku_id","order":1},{"displayType":"promotion","item":"6","item_type":"sku_id","order":2},{"displayType":"query","item":"6","item_type":"sku_id","order":3},{"displayType":"promotion","item":"4","item_type":"sku_id","order":4},{"displayType":"promotion","item":"5","item_type":"sku_id","order":5},{"displayType":"query","item":"2","item_type":"sku_id","order":6},{"displayType":"query","item":"2","item_type":"sku_id","order":7}],"page":{"during_time":15167,"last_page_id":"category","page_id":"activity"},"ts":1592102072994}',
               '$.page.last_page_id');
select get_json_object(
               '{"common":{"ar":"110000","ba":"iPhone","ch":"Appstore","md":"iPhone 8","mid":"mid_515","os":"iOS 13.3.1","uid":"391","vc":"v2.1.132"},"displays":[{"displayType":"query","item":"4","item_type":"sku_id","order":1},{"displayType":"promotion","item":"6","item_type":"sku_id","order":2},{"displayType":"query","item":"6","item_type":"sku_id","order":3},{"displayType":"promotion","item":"4","item_type":"sku_id","order":4},{"displayType":"promotion","item":"5","item_type":"sku_id","order":5},{"displayType":"query","item":"2","item_type":"sku_id","order":6},{"displayType":"query","item":"2","item_type":"sku_id","order":7}],"page":{"during_time":15167,"last_page_id":"category","page_id":"activity"},"ts":1592102072994}',
               '$.displays[5].order');
...
create function explode_json_array
    ...
    using jar 'hdfs://hadoop102:8020/user/hive/jars/dxyudtf.jar';
...
(
    ...
    from (select * from dwd_dim_user_info_his old where start_date<'2020-06-15') old
    left join
    (
        select
          *
...
--next_day函数
select next_day('2021-03-15','Monday');
--next_day函数使用只需记住一句话,一般用于获取下一个周几的日期
--下个周一一定是下周一
--下个周二等就不一定是下周二

--本周一是几号
select date_add(next_day(`current_date`(),'Monday'),-7);

--本周日是几号
select date_add(next_day(`current_date`(),'Monday'),-1);

--下周周一
select next_day(`current_date`(),'Monday');

--下周的周日
select date_add(next_day(`current_date`(),'Monday'),6);

--上周的周一是几号
select date_add(next_day(`current_date`(),'Monday'),-7*2);

--上周的周日是几号
select date_add(next_day(`current_date`(),'Monday'),-1-7);

--上上一周的周一是几号
select date_add(next_day(`current_date`(),'Monday'),-7*3);

--上上一周的周日是几号
select date_add(next_day(`current_date`(),'Monday'),-1-7*2);

--last_day函数（求当月最后一天日期）
select last_day('2020-06-25');
...
select
    ...
    if(old.login_count is null and new.login_count>0,'2020-06-14',old.login_date_first),--首次登陆
    if(new.login_count>0,'2020-06-14',old.login_date_last),--末次登陆时间
    nvl(old.login_count,0)+if(new.login_count>0,1,0), --累计登陆天数
    nvl(new.login_last_30d_count,0),
    if(old.user_id is null and new.order_count>0,'2020-06-14',old.order_date_first),--首次下单时间
    if(new.order_count>0,'2020-06-14',old.order_date_last),--末次下单时间
    nvl(old.order_count,0)+nvl(new.order_count,0),--累计下单次数
    nvl(old.order_amount,0)+nvl(new.order_amount,0),--累计下单金额
    nvl(new.order_last_30d_count,0),--累计30日下单次数
    nvl(new.order_last_30d_amount,0),--累计30日下单金额
    if(old.user_id is null and new.payment_count>0,'2020-06-14',old.payment_date_first),--首次支付时间
    if(new.payment_count>0,'2020-06-14',old.payment_date_last),--末次支付时间
    nvl(old.payment_count,0)+nvl(new.payment_count,0),--累计支付次数
    nvl(old.payment_amount,0)+nvl(new.payment_amount,0),--累计支付金额
    nvl(new.payment_last_30d_count,0),--累计30日支付次数
    nvl(new.payment_last_30d_amount,0)--累计30日支付金额
from dwt_user_topic old
full join
(
    select
    user_id,
    sum(if(dt='2020-06-14',login_count,0)) login_count,--当天登录次数
    sum(if(login_count >0,1,0)) login_last_30d_count,--最近三十天登陆天数
    sum(if(dt='2020-06-14',order_count,0)) order_count,--当天下单次数
    sum(if(dt='2020-06-14',order_amount,0)) order_amount,--当天下单次数
    sum(order_count) order_last_30d_count,--最近三十天下单次数
    sum(order_amount) order_last_30d_amount,--最近30天下单金额
    sum(if(dt='2020-06-14',payment_count,0)) payment_count,--当天支付次数
    sum(if(dt='2020-06-14',payment_amount,0)) payment_amount,--当天支付次数
    sum(payment_count) payment_last_30d_count,--最近三十天支付次数
    sum(payment_amount) payment_last_30d_amount--最近30天支付金额
...
--建表语句
--修复表数据--直接把数据导入到hdfs上需要手动修复数据，尤其是带分区的。
msck repair table dwt_activity_topic;
...
select
    '2020-06-25',--统计日期
    day_count.day_count,--当日用户数量
    wk_count.wk_count,--当周用户数量
    mn_count.mn_count,--当月用户数量
    if(date_add(next_day('2020-06-25','Monday'),-1)='2020-06-25','Y','N'),--'Y,N是否是周末，用于得到本周最终结果'
    if(last_day('2020-06-25')='2020-06-25','Y','N') --'Y,N书否是月末，用于得到本月最终结果'
from
(
    select
        '2020-06-25' dt,--统计日期
        count(*) day_count --当日用户数量
    from dwt_uv_topic
...
(
    select
        '2020-06-25' dt,--统计日期
        count(*) wk_count --当周用户数量
    from dwt_uv_topic
    where login_date_last>=date_add(next_day('2020-06-25','Monday'),-7)and login_date_last<=date_add(next_day('2020-06-25','Monday'),-1)
)wk_count
on day_count.dt=wk_count.dt
join
(
    select
        '2020-06-25' dt,--统计日期
        count(*) mn_count --当月用户数量
    from dwt_uv_topic
    where date_format(login_date_last,'yyyy-MM')=date_format('2020-06-25','yyyy-MM')
)mn_count
on day_count.dt=mn_count.dt;
...
select
    '2020-06-25',
    concat(date_add(next_day('2020-06-25','MO'),-7),'-',date_add(next_day('2020-06-25','MO'),-1)),
    count(*)
from
(
    select
    ...
    (
        ...
        (
            select
                ...
                dt,
                rk,
                date_add(dt,-rk) date_diff --用到等差数列知识
            from
            (
                select
                    mid_id,
...
(
    ...
    (
        select
            mid_id,
            dt,
            lead(dt,2,'9999-99-99') over(partition by mid_id order by dt) lead2_dt
        from
        (
            select
                mid_id,
...
select
    '2020-06-25',
    tmp_paymemnt.payment_count,
    tmp_paymemnt.payment_amount,
    tmp_paymemnt.payment_user_count,
    tmp_skucount.paymemnt_sku_count,
...
select
    '2020-06-25' dt,
    sum(if(payment_count>0,1,0)) paymemnt_sku_count
from dws_sku_action_daycount
where dt='2020-06-25'
)tmp_skucount
on tmp_paymemnt.dt=tmp_skucount.dt
join
(
    select
        '2020-06-25' dt,
...
on tmp_paymemnt.dt=tmp_time.dt;
...
with
tmp_order as
    ---如果这边表太复杂，你可以去dwd找，数据更精确，在dws 这个表left join 可能丢失数据。
(
    select
        user_id,
        order_stats_struct.sku_id sku_id,
        sum(order_stats_struct.order_count) order_count
    from dws_user_action_daycount lateral view explode(order_detail_stats) tmp as order_stats_struct --先侧写在炸裂 tmp表别名  order_stats_struct列别名 可能有多个商，分组
...
select
    ...
    date_format('2020-06-25','yyy-MM') stat_mn,
...
(
    ...
    group by tmp_order.user_id, tmp_sku.category1_id, tmp_sku.category1_name, tmp_sku.tm_id----可能买多次 第一题买6.第二天买了5，需要分组聚合
)tmp
...
select * from ads_uv_count where dt='2020-06-14';
select * from ads_new_mid_count;
select * from ads_silent_count where dt='2020-06-14';
select * from ads_back_count where dt='2020-06-14';
select * from ads_wastage_count where dt='2020-06-14';
select * from ads_user_retention_day_rate;
select * from ads_continuity_wk_count where dt='2020-06-14';
select * from ads_continuity_uv_count where dt='2020-06-14';
select * from ads_user_topic where dt='2020-06-14';
select * from ads_user_action_convert_day where dt='2020-06-14';
select * from ads_product_info where dt='2020-06-14';
select * from ads_product_sale_topN where dt='2020-06-14';
select * from ads_product_favor_topN where dt='2020-06-14';
select * from ads_product_cart_topN where dt='2020-06-14';
select * from ads_product_refund_topN where dt='2020-06-14';
select * from ads_appraise_bad_topN where dt='2020-06-14';
select * from ads_order_daycount where dt='2020-06-14';
select * from ads_payment_daycount where dt='2020-06-14';
select * from ads_sale_tm_category1_stat_mn;
select * from ads_area_topic where dt='2020-06-14';

----一共75张表
show tables ;
--75张表才能用这个脚本---ads20个指标，在linux界面跑脚本看是否有20个OK，再去DG取检查。是否数据更新
